...
export const getCategoryOffersService = async (page, perPage) => {
  ...
  const offers = await offerModel.find({ scope: "category" }).populate("category").skip((page-1)*perPage).limit(perPage)
...
export const getGlobalOffersService = async (page, perPage) => {
  const totalPosts = await offerModel.countDocuments({ scope: "global" });
    const offers = await offerModel.find({ scope: "global" }).skip((page-1)*perPage).limit(perPage)
    return {offers, totalPosts};
}

export const toggleOfferStatusService = async (id) => {
  const offer = await offerModel.findByIdAndUpdate(id, [{ $set: { isActive: { $not: "$isActive" } } }], { new: true })
  return offer;
}

export const editCategoryOfferService = async (id, body) => {
  const offer = await offerModel.findByIdAndUpdate(id, { $set: { discountValue: body.discountValue } }, { new: true })
  return offer
}
