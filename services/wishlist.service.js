...
export const addToWishlistService = async (userId, body) => {
    const isExist = await wishlistModal.find({ user: userId, product: body.product, variant: body.variant })
    if (isExist.length > 0) {
...
export const getWishlistService = async (userId) => {
    const wishlist = await wishlistModal.find({ user: userId }).populate("product").populate("variant")
